...
def decrypt(raw, password, salt_mask, key_sz, key_iter, hmac_key_sz, hmac_key_iter, page_sz, iv_sz, reserve_sz, hmac_sz):
    ...
    # decrypt pages
    for i in tqdm(range(0, int(len(raw) / 1024))):
...
def fast_stream_decrypt(filename_in: str, password: bytearray, filename_out: str):
    with open(filename_in, 'rb') as enc, open(filename_out, 'wb', buffering=config.page_sz * 16) as dec:
        enc_size = os.path.getsize(enc.name)

        dec.write(config.header)

        first_page = enc.read(config.page_sz)
        # derive key
        salt_sz = 16
        salt = first_page[:salt_sz]
        key, hmac_key = util.key_derive(salt, password, config.salt_mask, config.key_sz, config.key_iter, config.hmac_key_sz, config.hmac_key_iter)

        # decrypt file header, try with default page size
        page_sz, reserve_sz = decrypt_page_header(first_page, key, salt_sz, config.page_sz, config.iv_sz, config.reserve_sz)
        if page_sz < 0 or reserve_sz < 0:
            raise RuntimeError('failed to decide page size or reserve size.')

        enc.seek(0)
        for i in tqdm(range(0, int(enc_size / 1024))):
            page = enc.read(config.page_sz)
            if i == 0:
                page = page[salt_sz:]
            page_content = page[:-reserve_sz]
            reserve = page[-reserve_sz:]
            iv = reserve[:config.iv_sz]
            dec.write(util.decrypt(page_content, key, iv))
            dec.write(util.random_bytes(reserve_sz))
...
def try_get_reserve_size_for_specified_page_size(raw, key, salt_sz, page_sz, iv_sz, reserve_sz):
    ...
    if reserve_sz >= iv_sz:
        ...
        if util.is_valid_decrypted_header(first_page_dec) and page_sz == util.get_page_size_from_database_header(
                raw[:salt_sz] + first_page_dec) and reserve_sz == util.get_reserved_size_from_database_header(raw[:salt_sz] + first_page_dec):
            return reserve_sz
